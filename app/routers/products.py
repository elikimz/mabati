...
@cache_response(prefix="products", ttl=600)  # 10 min cache
...
@cache_response(prefix="products:featured", ttl=900)  # 15 min cache
...
@cache_response(prefix="products:slug", ttl=900)  # 15 min cache
...
@cache_response(prefix="products:id", ttl=900)  # 15 min cache
...
@cache_response(prefix="products:related", ttl=600)  # 10 min cache
...
# ── Admin mutations (with cache invalidation) ────────────────────────────────
...
async def create_product(
    ...
    await invalidate_cache(prefix="products")
    return result
...
async def add_product_image(
    ...
    service = ProductService(db)
    result = await service.add_image(product_id, data)
    await invalidate_cache(prefix="products")
    return result
